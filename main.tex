...
\centering
\small
\begin{tabular}{|p{2cm}|p{2.5cm}|p{2.2cm}|p{2cm}|p{2.2cm}|p{2cm}|p{2.3cm}|}
\hline
\textbf{Vergleichs- \newline methode} & \textbf{Wann benutzt?} & \textbf{Voraus- \newline setzungen} & \textbf{Vorteile} & \textbf{Nachteile} & \textbf{Unterschiede zu anderen Methoden} & \textbf{Weitere \newline Kriterien} \\
\hline
\textbf{ANOVA} & Vergleich von mehr als zwei Gruppen, um festzustellen, ob mindestens eine Gruppe sich signifikant unterscheidet & Normalverteilung in den Gruppen; \newline Homogene Varianzen; \newline Unabhängige Stichproben & Effizient bei mehreren Gruppen; \newline Reduziert Fehler durch multiple Tests & Nur Signifikanz, keine spezifischen Unterschiede; \newline Voraussetzungen oft schwer zu erfüllen & Vergleich zu t-Tests (nur für zwei Gruppen); \newline Erfordert Post-hoc-Tests bei Signifikanz & Bei Verletzung der Voraussetzungen: Alternativen wie nichtparametrische Tests (z.B. Kruskal-Wallis) \\
\hline
\textbf{Kruskal-Wallis-Test} & Nichtparametrischer Vergleich mehrerer Gruppen, wenn Voraussetzungen für ANOVA nicht erfüllt sind & Keine Normalverteilung; \newline Unabhängige Stichproben & Robust gegen Verletzungen der Normalverteilung; \newline Einfach anzuwenden & Weniger sensitiv; \newline Keine Angabe zu spezifischen Unterschieden & Vergleich zu ANOVA: keine Annahme der Normalverteilung & Für ordinal skalierte Daten geeignet \\
\hline
\textbf{Post-hoc-Tests} & Nach einem signifikanten ANOVA oder Kruskal-Wallis, um spezifische Gruppenunterschiede zu identifizieren & Signifikanter Gesamteffekt (z.B. ANOVA); \newline Mehrere Gruppen & Identifikation spezifischer Unterschiede; \newline Kontrolle der Fehlerwahrscheinlichkeit & Erhöhte Komplexität; \newline Kann konservativ sein (z.B. Bonferroni) & Ergänzung zu ANOVA/ Kruskal-Wallis, keine eigenständige Methode & Wichtig bei mehreren Vergleichen, um Fehler zu kontrollieren \\
\hline
\textbf{Multiple Tests} & Vergleich aller Gruppenpaare, wenn nur wenige Gruppen vorhanden sind & Unabhängige Stichproben; \newline Normalverteilung (bei parametrischen Tests) & Einfach bei wenigen Vergleichen; \newline Direkte Unterschiede sichtbar & Erhöhtes Risiko für Fehler 1. Art (falsch positive Ergebnisse) & Vergleich zu globalen Tests (ANOVA, Kruskal-Wallis) & Fehlerkontrolle notwendig (z.B. Bonferroni, Holm) \\
\hline
\textbf{FDR} & Korrektur bei vielen Tests, um die Rate der falsch positiven Ergebnisse zu kontrollieren & Mehrere Tests; \newline Unabhängige oder abhängige Tests & Weniger konservativ als Bonferroni; \newline Beibehaltung der Power & Komplexere Berechnung; \newline Kontrolle der FDR, nicht des Familienfehlerniveaus & Vergleich zu Bonferroni: weniger streng, mehr Power & Besonders bei Hochdurchsatzdaten (z.B. Genexpression) sinnvoll \\
\hline
\end{tabular}
\end{table}
...
\subsection{FDR (False Discovery Rate)}
Statt „kein einziger Fehlalarm“ zu garantieren (wie Bonferroni) kontrolliert FDR den \textbf{Anteil falscher Entdeckungen unter allen signifikanten Ergebnissen}.

\subsubsection{Benjamini–Hochberg (BH)}
BH baut eine adaptive, steigende Schwelle $T_i$ auf den sortierten p-Werten.
\textbf{Gegeben:}
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item $m$ Tests mit $p$-Werten $p_1, \ldots, p_m$
    \item gewünschte FDR-Rate $\alpha$ (z.\,B.\ 0.05)
\end{itemize}
\textbf{Algorithmus:}
\begin{enumerate}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item Sortiere die $p$-Werte: $p_{(1)} \le p_{(2)} \le \cdots \le p_{(m)}$
    \item Für jedes $i = 1, \ldots, m$ berechne die BH-Schwelle $T_i = \frac{i}{m} \alpha$
    \\$\rightarrow$ $T_i$ steigt mit $i$ (später „dürfen“ $p$-Werte grösser sein).
    \item Finde das grösste $i$, für das $p_{(i)} \le T_i$
    \item Dann sind alle Hypothesen mit $p_{(1)}, \ldots, p_{(i)}$ signifikant nach FDR.
\end{enumerate}
Visuell (FDR-Linie):
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item sortierte $p$-Werte als Punkte
    \item BH-Linie: $T_i = i/m \cdot \alpha$ (ansteigende Linie)
    \item Punkte \textbf{unter} der Linie $\rightarrow$ signifikant
    \item Punkte \textbf{über} der Linie $\rightarrow$ nicht signifikant
\end{itemize}

\subsubsection{FDR-korrigierte p-Werte}
In der Praxis (z.\,B.\ \texttt{statsmodels.multitest.multipletests}):
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item du gibst eine Liste von $p$-Werten rein,
    \item das BH-Verfahren berechnet \textbf{FDR-korrigierte p-Werte} $\tilde{p}_j$,
    \item dann gilt:
    \begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item $\tilde{p}_j \le \alpha \Leftrightarrow$ Test $j$ ist signifikant nach FDR.
    \end{itemize}
\end{itemize}
Diese korrigierten $p$-Werte sind so skaliert, dass der erwartete Anteil falscher Discoveries $\approx \alpha$ bleibt.
\textbf{Vorteil:}
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item weniger streng als Bonferroni $\Rightarrow$ mehr echte Effekte
    \item trotzdem sinnvolle Kontrolle der False Positives.
\end{itemize}
\textbf{Ein Test = eine Hypothesenprüfung = ein p-Wert}

\section{Regression}
Regression beschreibt, wie X sich verändert, wenn Y sich verändert.

\subsection{Einfache Lineare Regression}
Regression ist ein Verfahren, um den gerichteten Zusammenhang zwischen einer unabhängigen Variable (X) und einer abhängigen Variable (Y) zu modellieren. $\rightarrow$ Wir nähern die Realität (die oft nicht perfekt linear ist) durch eine lineare Funktion an, um ein einfaches, interpretierbares Modell zu erhalten.

\subsubsection{Regression vs. Korrelation}
Während die Korrelation nur die Stärke eines Zusammenhangs misst, modelliert die Regression die Art des Zusammenhangs.\\
\begin{tabular}{ll|l}
\textbf{Merkmal} & \textbf{Korrelation ($r$)} & \textbf{Regression ($R^2, \beta$)} \\[1ex]
\textbf{Symmetrie} & 
Symmetrisch: $\rho(X,Y) = \rho(Y,X)$ &
\textbf{Gerichtet:} $X \rightarrow Y$ (Ursache $\rightarrow$ Wirkung Annahme) \\[2ex]
\textbf{Fragestellung} &
„Wie stark hängen X und Y zusammen?“ &
„Wie verändert sich Y, wenn ich X ändere?“ \\[2ex]
\textbf{Output} &
Eine Zahl zwischen -1 und 1 &
Eine Funktionsgleichung (Gerade) \\[2ex]
\textbf{Einheit} &
Einheitslos &
Hängt von den Einheiten von X und Y ab \\
\end{tabular}

\subsubsection{Das Lineare Modell}
Die einfache lineare Regression beschreibt Y als lineare Funktion von X plus Fehler.\\
\textbf{Die Modellgleichung:} $Y = \beta_0 + \beta_1 X + \epsilon$
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item $\beta_0$ \textbf{(Achsenabschnitt / Intercept)}:
    \begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item Der erwartete Wert von $Y$, wenn $X = 0$ ist.
        \item \textit{Interpretation:} „Der Basiswert ohne Einfluss von X.“
    \end{itemize}
    \item $\beta_1$ \textbf{(Steigung / Slope)}:
    \begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item Die Veränderung von $Y$, wenn $X$ um \textbf{eine Einheit} steigt.
        \item \textit{Interpretation:} „Wenn X um 1 steigt, steigt/sinkt Y durchschnittlich um $\beta_1$.“
    \end{itemize}
    \item $\epsilon$ \textbf{(Fehlerterm / Residuum)}:
    \begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item Alles, was das Modell nicht erklären kann (Rauschen, Zufall).
    \end{itemize}
\end{itemize}
\textbf{Achtung:} Ein lineares Modell ist oft nur eine Approximation der Realität. 
Wir nutzen es, weil es einfach und interpretierbar ist.

\subsubsection{Schätzung: OLS (Ordinary Least Squares)}
Wie finden wir die "beste" Gerade durch die Punktewolke? Methode: Kleinste-Quadrate-Schätzung (OLS).
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Ziel:} Minimiere die Summe der quadrierten Abweichungen (Fehler).
    \item \textbf{Formel:} Minimiere $RSS = \sum_{i=1}^{n} (y_i - \hat{y}_i)^2$
    \item \textbf{Warum werden die Fehler quadriert?}
    \begin{enumerate}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item Damit sich positive und negative Fehler nicht gegenseitig aufheben.
        \item Große Abweichungen werden stärker bestraft („Ausreißer ziehen die Gerade an“).
    \end{enumerate}
\end{itemize}

\subsubsection{Residuen}
Nachdem wir das Modell geschätzt haben, müssen wir prüfen, ob es valide ist. Das tun wir über die Residuen.\\
Das Residuum $e_i$ ist die Differenz zwischen der Realität und der Modellvorhersage.\\
$\textit{Residuum} = \text{beobachtetes } Y - \text{vorhergesagtes } \hat{Y}$\\
Wir wollen, dass die Residuen zufällig aussehen (kein Muster). Das bedeutet, das Modell hat alle systematischen Informationen aus den Daten herausgefiltert, und übrig bleibt nur noch zufälliges Rauschen.\\
Diagnose: Was wir in den Residuen \textbf{NICHT} sehen wollen\\
Wenn die Residuen Muster zeigen, haben wir ein Problem:\\
\begin{tabular}{l l p{8cm}}
\textbf{Muster im Plot} & \textbf{Diagnose (Problem)} & \textbf{Bedeutung} \\[6pt]
Kurve / Bogen & Nichtlinearität & 
Der Zusammenhang ist nicht linear (z.B. quadratisch). Unser einfaches lineares Modell passt nicht. \\[10pt]
Trichterform & Heteroskedastizität & 
Die Varianz der Fehler ist nicht konstant (z.B. werden Fehler bei großen Werten größer). \\[10pt]
Einzelne Punkte weit weg & Ausreißer & 
Einzelne Datenpunkte verzerren das Modell massiv. \\
\end{tabular}

\subsection{Zerlegung der Variation (TSS, ESS, RSS)}
Um zu messen, wie "gut" das Modell ist, zerlegen wir die gesamte Streuung der Daten in zwei Teile: Was wir erklären können und was Fehler bleibt.\\
Die „Magische Formel“ der Varianzzerlegung lautet: $TSS = ESS + RSS$\\
Hier die Definitionen für deine Formelsammlung:\\
\begin{tabular}{l l l p{8cm}}
\textbf{Kürzel} & \textbf{Name} & \textbf{Formel} & \textbf{Erklärung} \\[6pt]
TSS & Total Sum of Squares &
$\sum (y_i - \bar{y})^2$ &
\textbf{Gesamtvariation:} Wie stark schwanken die Daten insgesamt um ihren Mittelwert? (Ohne Modell). \\[12pt]
RSS & Residual Sum of Squares &
$\sum (y_i - \hat{y}_i)^2$ &
\textbf{Unerklärte Variation:} Der Fehler, den das Modell noch macht. (Je kleiner, desto besser). \\[12pt]
ESS & Explained Sum of Squares &
$\sum (\hat{y}_i - \bar{y})^2$ &
\textbf{Erklärte Variation:} Wie viel der Streuung kann das Modell „einfangen“? (Je größer, desto besser). \\
\end{tabular}

\subsection{Gütemaße: $R^2$ und RSE}
Wie fassen wir diese Variationen nun in einer verständlichen Kennzahl zusammen?
\begin{enumerate}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Das Bestimmtheitsmaß ($R^2$)}\\
    Das $R^2$ ist die wohl wichtigste Kennzahl für die Kommunikation von Ergebnissen.
    \begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item \textbf{Formel:} $R^2 = 1 - \frac{RSS}{TSS} 
            \quad \text{(oder alternativ } 
            \frac{ESS}{TSS}\text{)}$
        \item \textbf{Wertebereich:} 0 bis 1 (oder 0\% bis 100\%).
        \item \textbf{Interpretation:} Gibt den \textbf{Anteil der Variation} an, der durch das Modell erklärt wird.
        \begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
            \item \textit{Beispiel:} $R^2 = 0.62$ bedeutet: „62\% der Unterschiede in der Lebenserwartung werden durch die Mortalität erklärt.“
        \end{itemize}
    \end{itemize}
    \textbf{Ist ein hohes $R^2$ immer gut? (Prüfungsfalle!)}  \\
    Nicht zwingend.
    \begin{enumerate}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item \textbf{Kontextabhängig:}  
        In der Physik erwarten wir $R^2 > 0.95$.  
        In Sozialwissenschaften oder bei echtem Nutzerverhalten („Noisy Real-World Data“) ist oft schon $R^2 > 0.20$ gut.
        \item \textbf{Overfitting:}  
        Ein extrem hohes $R^2$ kann bedeuten, dass das Modell den Zufall mitlernt, statt den Trend.
    \end{enumerate}
    \item \textbf{Residual Standard Error (RSE)}\\
    Während $R^2$ eine Prozentzahl ist, gibt der RSE den Fehler in der \textbf{Einheit der Daten} an.
    \begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item \textbf{Formel:} $RSE = \sqrt{\frac{RSS}{n - 2}}$
        \item \textbf{Interpretation:}  
        Die \textbf{typische Abweichung} eines Datenpunkts von der Regressionsgeraden.
        \item \textbf{Freiheitsgrade:}  
        Wir teilen durch $n - 2$, weil wir für die Berechnung bereits 2 Parameter ($\beta_0$ und $\beta_1$) aus den Daten geschätzt haben.  
        Das kostet uns 2 Freiheitsgrade.
    \end{itemize}
\end{enumerate}

\subsection{Modellannahmen}
Damit wir den Ergebnissen einer linearen Regression (und vor allem den p-Werten und Konfidenzintervallen) vertrauen können, müssen vier zentrale Annahmen erfüllt sein. Wenn diese verletzt sind, ist das Modell unzuverlässig.
\begin{enumerate}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Unabhängigkeit}: Die Fehler (Residuen) dürfen nichts miteinander zu tun haben (keine Muster entlang der Zeit oder Reihenfolge).
    \item \textbf{Linearität}: Der Zusammenhang zwischen X und Y muss tatsächlich linear sein. Die Residuen sollten zufällig um 0 streuen.
    \item \textbf{Homoskedastizität (Gleiche Varianz)}: Die Streuung der Fehler muss über den gesamten Bereich der Vorhersage gleich bleiben (keine Trichterform).
    \item \textbf{Normalität}: Die Residuen sollten annähernd normalverteilt sein (wichtig für t-Tests und Inferenz).
\end{enumerate}

\subsection{Diagnoseplots}
Wir nutzen drei Haupt-Plots, um diese Annahmen zu prüfen. Du musst in der Lage sein, "schlechte" Plots zu erkennen.
\begin{enumerate}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Residuen-vs-Fit Plot}\\
    Hier werden die Residuen ($y$-Achse) gegen die vorhergesagten Werte ($\hat{y}$, $x$-Achse) geplottet.
    \begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item \textbf{Ideal:} Eine zufällige Punktewolke („Sternenhimmel“) ohne Struktur um die 0-Linie.
        \item \textbf{Problem 1: Nichtlinearität:}  
        Die Punkte bilden eine \textbf{Kurve} (z.\,B. Parabel/U-Form).  
        $\rightarrow$ Das lineare Modell passt nicht.
        \item \textbf{Problem 2: Heteroskedastizität:}  
        Die Punkte bilden einen \textbf{Trichter} (werden nach rechts immer breiter).  
        Das heißt: bei großen Werten macht das Modell größere Fehler.
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.45\linewidth]{img/QQResiduen.png}
        \end{figure}
    \end{itemize}
    \item \textbf{QQ-Plot (Normal Probability Plot)}\\
    Prüft die \textbf{Normalverteilung} der Residuen.
    \begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item \textbf{x-Achse:} Theoretische Quantile (was wir bei perfekter Normalverteilung erwarten).
        \item \textbf{y-Achse:} Beobachtete Quantile (unsere echten Residuen).
        \item \textbf{Ideal:} Alle Punkte liegen auf der roten Diagonalen.
        \item \textbf{Problem:} Punkte weichen stark ab, bilden eine S-Form oder Kurve  
        $\rightarrow$ Keine Normalverteilung (z.\,B. „Heavy Tails“ / Ausreißer).
    \end{itemize}
        \begin{figure}[H]
            \centering
            \includegraphics[width=0.7\linewidth]{img/QQResiduenNN.png}
        \end{figure}
    \item \textbf{Cook's Distance}\\
    Identifiziert \textbf{einflussreiche Punkte}, die das Modell verzerren könnten.\\
    \textbf{Kombination aus:}
    \begin{enumerate}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item Größe des Residuums (Wie falsch liegt der Punkt?).
        \item Leverage (Wie extrem ist der $x$-Wert? Liegt er weit außen?).
    \end{enumerate}  
    Hohe Balken zeigen Punkte, die die Regressionsgerade „zu sich ziehen“.
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\linewidth]{img/CooksDistance.png}
    \end{figure}
\end{enumerate}

\subsection{Grenzen der Regression}
\begin{enumerate}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Keine Kausalität beweisen}: Nur weil X und Y korrelieren, heißt das nicht, dass X Ursache für Y ist.
    \item \textbf{Keine automatische Bereinigung}: Ausreißer werden nicht automatisch ignoriert (OLS ist anfällig für sie!).
    \item \textbf{Komplexe Zusammenhänge}: Starke Nichtlinearitäten werden durch eine Gerade falsch dargestellt.
\end{enumerate}

\section{Multiple Regression}
Die Statistik für Data Science lässt sich grob in drei Phasen unterteilen, wobei wir uns nun in der prädiktiven Modellierung befinden:
\begin{enumerate}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Explorative Phase (Deskriptive Statistik)}: Daten beschreiben und verstehen (Verteilungen, Ausreisser).
    \item \textbf{Unsicherheitsphase (Inferenzstatistik)}: Schätzen und Entscheiden (Standardfehler, Konfidenzintervalle, Hypothesentests).
    \item \textbf{Modellierungsphase (Prädikative Statistik)}: Zusammenhänge modellieren und Vorhersagen treffen.
    \begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item Bisher: Einfache Regression (1 Prädiktor).
        \item Neu: Multiple Regression (mehrere Prädiktoren), Dummy-Codierung, Umgang mit Konfundierung und Panelmodelle.
    \end{itemize}
\end{enumerate}

\subsection{Multiple Lineare Regression}
Die einfache lineare Regression ($Y = \beta_0 + \beta_1 X$) ist oft zu simpel für die Realität, da meist mehrere Faktoren eine Zielgrösse beeinflussen.\\
Wir erweitern die Gleichung um weitere Prädiktoren\\ $X_2, X_3, \ldots, X_k\text{: } Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \cdots + \beta_k X_k + \varepsilon$
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item $Y$: Abhängige Variable (Zielgrösse).
    \item $X_1, \ldots, X_k$: Unabhängige Variablen (Prädiktoren).
    \item $\beta_0$: Achsenabschnitt (Intercept) – Wert von $Y$, wenn alle $X_i = 0$.
    \item $\beta_1, \ldots, \beta_k$: Partielle Regressionskoeffizienten.
    \item $\varepsilon$: Fehlerterm (Residuum).
\end{itemize}

\subsubsection{Interpretation der Koeffizienten (Ceteris Paribus)}
In der multiplen Regression ändert sich die Interpretation der Steigungskoeffizienten entscheidend:
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item $\beta_j$: Die Veränderung von $Y$, wenn $X_j$ um eine Einheit steigt, \textit{während alle anderen Prädiktoren konstant gehalten werden} (ceteris paribus).
    \begin{itemize}
        \item \textit{Beispiel}: Einfluss der Wohnfläche auf den Preis, \textit{bei gleicher Lage und gleichem Baujahr}.
    \end{itemize}
    \item Dies isoliert den "bereinigten" Effekt einer Variablen.
\end{itemize}

\subsubsection{Warum Multiple Regression? (Vorteile)}
\begin{enumerate}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Erklärungskraft erhöhen ($R^2$)}: Mehr relevante Infos führen zu besseren Vorhersagen.
    \item \textbf{Konfundierung kontrollieren}: Wir können den Einfluss von Störvariablen (Z) herausrechnen, um den "wahren" Effekt von X auf Y zu isolieren (verhindert Simpson-Paradox).
    \item \textbf{Genauere Schätzungen}: Der Standardfehler der Koeffizienten kann sinken, wenn Rauschen durch andere Variablen erklärt wird.
\end{enumerate}

\subsubsection{Adjusted $R^2$ (Das korrigierte Bestimmtheitsmass)}
Das normale $R^2$ steigt \textit{immer}, wenn man eine neue Variable hinzufügt – selbst wenn diese sinnlos ist (Overfitting-Gefahr).
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Lösung}: Adjusted $R^2$ ($R^2_{\text{adj}}$).
    \item Es bestraft Komplexität: Es steigt nur, wenn der neue Prädiktor das Modell stärker verbessert, als durch reinen Zufall zu erwarten wäre.
    \item \textbf{Formel}: 
   $ R^2_{\text{adj}} = 1 - (1 - R^2)\frac{n - 1}{n - k - 1}$
    \begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item ($n =$ Anzahl Beobachtungen, $k =$ Anzahl Prädiktoren).
    \end{itemize}
    \item \textbf{Nutzung}: Zum Vergleich von Modellen mit unterschiedlich vielen Variablen.
\end{itemize}

\section{Dummy-Codierung}
Bisher waren unsere Prädiktoren meist kontinuierlich (z. B. Wohnfläche). Wie bauen wir kategoriale Variablen (z. B. „Stadt“: Zürich, Genf, Bern) in die Regression ein?\\
Computer können nicht mit Text rechnen. Wir wandeln Kategorien in binäre 0/1-Variablen um, sogenannte Dummy-Variablen.
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Regel}: Für eine Variable mit k Kategorien benötigen wir $k-1$ Dummies.
    \item \textbf{Referenzkategorie}: Die Kategorie, die keine eigene Dummy-Variable bekommt (sie ist dann 0 in allen anderen Dummies). Sie dient als Basisvergleich.
\end{itemize}
Nehmen wir ein Modell für Hauspreise mit der kategorialen Variable \textit{Lage} (Kategorien: \textit{Stadt, Land}). Wir wählen \textit{Land} als Referenz.\\
$Y = \beta_0 + \beta_1 \cdot \text{Wohnfläche} + \beta_2 \cdot \text{Stadt\_Dummy} + \varepsilon$
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Stadt\_Dummy}: Ist 1, wenn das Haus in der Stadt liegt, sonst 0.
    \item $\beta_0$ \textbf{(Intercept)}: Preis eines Hauses auf dem \textit{Land} (Referenz), wenn Wohnfläche $= 0$.
    \item $\beta_2$ \textbf{(Dummy-Koeffizient)}: Der \textit{Niveauunterschied}. Er zeigt, wie viel teurer ein Haus in der Stadt im Vergleich zum Land ist, \textit{bei gleicher Wohnfläche} (ceteris paribus).
    \begin{itemize}
        \item Ist $\beta_2$ signifikant positiv $\rightarrow$ Stadt ist teurer als Land.
    \end{itemize}
\end{itemize}
\textbf{Achtung "Dummy Trap"}: Würden wir für \textit{alle} $k$ Kategorien eine Dummy-Variable erstellen, entstünde perfekte Multikollinearität (siehe unten), und das Modell wäre nicht mehr berechenbar.

\subsection{Multikollinearität \& Konfundierung}

\subsubsection{Konfundierung (Omitted Variable Bias)}
Ein häufiges Problem: Wir lassen eine wichtige Variable $Z$ weg, die sowohl $X$ als auch $Y$ beeinflusst.
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Folge}: Der Effekt von $Z$ wird fälschlicherweise X zugeschrieben. Der Koeffizient von $X$ ist verzerrt (biased).
    \item \textbf{Lösung}: Aufnahme der Störvariable in die multiple Regression kontrolliert diesen Effekt ("Herausrechnen").
\end{itemize}

\subsubsection{Multikollinearität (Redundanz)}
Tritt auf, wenn zwei oder mehr Prädiktoren sehr stark miteinander korrelieren (z.~B. \textit{Wohnfläche in $m^2$} und \textit{Wohnfläche in ft$^2$}, oder \textit{Alter} und \textit{Berufserfahrung}).

\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Das Problem}: Das Modell kann nicht mehr unterscheiden, welcher Prädiktor für die Änderung in $Y$ verantwortlich ist.
    \begin{itemize}
        \item $R^2$ ist oft hoch (Modell erklärt viel), aber ...
        \item ... die einzelnen p-Werte sind hoch (nicht signifikant), weil die Standardfehler der Koeffizienten „aufgebläht“ werden.
    \end{itemize}
    \item \textbf{Diagnose: VIF (Variance Inflation Factor)}
\end{itemize}
$VIF_j = \frac{1}{1 - R_j^2}$\\
Hierbei ist $R_j^2$ das Bestimmtheitsmass einer Regression, bei der Prädiktor $X_j$ durch alle anderen Prädiktoren erklärt wird.
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Faustregel}: $VIF > 5$ (manchmal $> 10$) deutet auf problematische Multikollinearität hin.
    \item \textbf{Lösung}: Einen der korrelierten Prädiktoren entfernen oder Variablen zusammenfassen.
\end{itemize}
\textbf{Was tun bei Multikollinearität?}
Wenn du einen hohen VIF feststellst, gibt es drei Hauptstrategien:
\begin{enumerate}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Variable entfernen}: Entferne den Prädiktor mit dem höchsten VIF (oder jenen, der theoretisch weniger relevant ist)
    \item \textbf{Variablen zusammenfassen (Feature Engineering)}: Wenn zwei Variablen fast das Gleiche messen (z. B. "Kundenzufriedenheit Umfrage 1" und "Kundenzufriedenheit Umfrage 2"), bilde einen Mittelwert oder Score aus beiden.
    \item \textbf{Datenmenge erhöhen ($n$ vergrössern)}: Multikollinearität ist oft ein Problem von zu wenigen Daten ("Micronumerosity"). Mehr Daten verringern die Standardfehler, auch wenn die Korrelation bestehen bleibt (in der Praxis aber oft schwer umsetzbar).
\end{enumerate}


\subsubsection{VIF: Variance Inflation Factor}
Der Variance Inflation Factor ($VIF_j$) gibt an, um welchen Faktor die Varianz (und damit das Quadrat des Standardfehlers) eines Regressionskoeffizienten aufgebläht wird, nur weil dieser Prädiktor mit anderen korreliert.
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item $VIF = 1$: Keine Multikollinearität (Idealfall). Die Variable ist unabhängig von allen anderen Prädiktoren.
    \item $1 < VIF < 5$: Moderate Korrelation. In der Regel unproblematisch.
    \item $VIF > 5$: Starke Multikollinearität. Die Schätzungen werden ungenau, p-Werte sind oft nicht vertrauenswürdig.
    \item $VIF > 10$: Kritische Multikollinearität. Das Modell ist instabil; es besteht dringender Handlungsbedarf.
\end{itemize}

\section{Panelmodelle}
Bisher haben wir meist Querschnittsdaten betrachtet (viele Beobachtungen zu einem Zeitpunkt, z. B. Immobilienpreise 2024). In der Realität wollen wir aber oft Entwicklungen über die Zeit verstehen.\\
\textbf{Paneldaten} kombinieren \textbf{Querschnitt} (viele Einheiten) und Zeitreihe (Entwicklung über die Zeit). Wir beobachten dieselben Einheiten (Entities) wiederholt.
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Struktur}: Einheit $i$ zum Zeitpunkt $t$.
    \item \textbf{Notation}: $Y_{it}$
    \begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item $i = 1, \ldots, N$ (Einheiten, z.~B. Länder, Firmen, Personen)
        \item $t = 1, \ldots, T$ (Zeitpunkte, z.~B. Jahre, Quartale, Tage)
    \end{itemize}
\end{itemize}

\subsection{Die zwei Arten der Variation}
Bei Paneldaten gibt es zwei Quellen, warum sich Daten unterscheiden können. Das Verständnis dieser Unterscheidung ist zentral:
\begin{enumerate}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Between-Variation (Zwischen den Einheiten)}:
    \begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item Unterschiede zwischen den Subjekten.
        \item \textit{Beispiel}: "Die Schweiz ist im Durchschnitt reicher als Portugal."
        \item Dies sind oft langfristige, strukturelle Unterschiede (Kultur, Geografie).
    \end{itemize}
    \item \textbf{Within-Variation (Innerhalb der Einheit)}:
    \begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
        \item Veränderungen einer Einheit über die Zeit.
        \item \textit{Beispiel}: "Die Schweiz ist 2022 reicher geworden im Vergleich zur Schweiz 2021."
        \item Dies zeigt die Dynamik und wie eine Einheit auf Veränderungen reagiert.
    \end{itemize}
\end{enumerate}
\textbf{Warum ist das wichtig?}\\ Wenn wir einfach eine normale Regression über alle Daten machen (Pooled OLS), vermischen wir diese beiden Effekte. Das führt oft zu falschen Schlüssen (vergleichbar mit dem Simpson-Paradox).

\subsection{Das Fixed Effects (FE) Modell}
Das \textbf{Fixed Effects Modell} ist der Standardansatz in der Ökonometrie und Data Science, um mit Paneldaten zu arbeiten. Es nutzt primär die \textit{Within-Variation}.\\
$Y_{it} = \beta_1 X_{it} + \alpha_i + \epsilon_{it}$
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item $\beta_1 X_{it}$: Der Effekt der Variable $X$ (wie gewohnt).
    \item $\alpha_i$ (\textbf{Der Fixed Effect}): Das ist der Trick! Jede Einheit $i$ bekommt ihren eigenen, individuellen Achsenabschnitt (Intercept).
    \item $\epsilon_{it}$: Der verbleibende Fehlerterm.
\end{itemize}
\textbf{Was leistet das Modell ($\alpha_i$)?}  \\
Das $\alpha_i$ fängt \textbf{alle Eigenschaften einer Einheit} auf, die sich über die Zeit nicht ändern (zeit-invariant), selbst wenn wir diese gar nicht gemessen haben!
\begin{itemize}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textit{Beispiel Länder}: Das Modell kontrolliert automatisch für Geografie, Klima, Geschichte oder Kultur eines Landes.
    \item \textit{Beispiel Personen}: Es kontrolliert für genetische Veranlagung oder Intelligenz (sofern konstant).
\end{itemize}
\textbf{Vorteile des FE-Modells:}
\begin{enumerate}[itemsep=0.4em, topsep=0.4em, parsep=0em, labelsep=0.5em, leftmargin=*, align=parleft]
    \item \textbf{Eliminierung von Omitted Variable Bias}:  
    Wir müssen uns keine Sorgen machen, dass wir zeit-konstante Störvariablen vergessen haben. Das Modell „rechnet sie heraus“.

    \item \textbf{Fokus auf Veränderung}:  
    Es beantwortet die Frage:  
    \textit{„Wenn $X$ für eine bestimmte Einheit steigt, wie verändert sich dann $Y$ für diese Einheit?“}
\end{enumerate}

\subsubsection{Vergleich: OLS vs. Fixed Effects}
\begin{table}[H]
\centering
\begin{tabular}{p{3.2cm} p{6cm} p{6cm}}
\textbf{Merkmal} & \textbf{Pooled OLS (Normale Regression)} & \textbf{Fixed Effects (FE) Modell} \\[6pt]
\textbf{Gleichung} &
$Y_{it} = \beta_0 + \beta_1 X_{it} + \epsilon$
 &
$Y_{it} = \alpha_i + \beta_1 X_{it} + \epsilon_{it}$
 \\[10pt]
\textbf{Annahme} &
Alle Einheiten sind „gleich“; keine individuelle Heterogenität. &
Jede Einheit hat ein eigenes Niveau ($\alpha_i$). \\[10pt]
\textbf{Umgang mit } $\alpha_i$ &
Ignoriert ($\alpha_i$ landet im Fehlerterm). &
Schätzt oder eliminiert $\alpha_i$. \\[10pt]
\textbf{Gefahr} &
\textbf{Omitted Variable Bias} (sehr hoch), wenn individuelle Merkmale mit $X$ korrelieren. &
Keine Verzerrung durch zeitinvariante Störvariablen. \\[10pt]
\textbf{Interpretation} &
Vermischt Äpfel \& Birnen (Between \& Within). &
Vergleicht nur „Äpfel mit sich selbst“ (Only Within). \\[10pt]
\textbf{Nachteil} &
Ergebnisse oft falsch/verzerrt. &
Kann Einfluss von konstanten Variablen (z.\,B. Geschlecht, Lage) nicht messen. \\
\end{tabular}
\end{table}
